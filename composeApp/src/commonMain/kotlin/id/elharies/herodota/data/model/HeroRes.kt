...
data class HeroRes(
    ...
    companion object {
        fun HeroRes.mapToListAttribute(): MutableList<Attribute> {
            return mutableListOf(
                Attribute(label = "Strength", valueAttribute = this.baseStr),
                Attribute(label = "Agility", valueAttribute = this.baseAgi),
                Attribute(label = "Intelligence", valueAttribute = this.baseInt),
            )
        }

        fun HeroRes.mapToHealthHud(): HudModel {
            return HudModel(
                hudValue = this.baseHealth,
                hudRegen = this.baseHealthRegen ?: 0.0
            )
        }

        fun HeroRes.mapToManaHud(): HudModel {
            return HudModel(
                hudValue = this.baseMana,
                hudRegen = this.baseManaRegen
            )
        }
    }
}